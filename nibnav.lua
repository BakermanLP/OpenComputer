...
function nibnav.getCost(x, y, z)
    ...
    ourX, ourY, ourZ = nibnav.getPosition()

    stepsX = math.abs(ourX - x)
    stepsY = math.abs(ourY - y)
    stepsZ = math.abs(ourZ - z)

    facing=nibnav.getFacing()

    stepCost = stepsX + stepsY + stepsZ

    zFacing = ourZ < z and sides.posz or sides.negz
    if stepsZ > 0 then
        if zFacing ~= facing then
            turnCostZ = math.abs(nibnav.sideLookup.turn[facing][zFacing])
        end
    end

    xFacing = ourX < x and sides.posx or sides.negx
    if stepsX > 0 then
        if xFacing ~= facing then
            turnCostX = math.abs(nibnav.sideLookup.turn[facing][xFacing])
        end
        if turnCostX == 2 and turnCostZ ~= 0 then
          turnCostX = 1
        end
    end
    turnCost = turnCostX + turnCostZ

    cost = stepCost * robMoveCost + turnCost * robTurnCost
